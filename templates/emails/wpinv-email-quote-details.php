...
<div id="wpinv-email-details">
    <h3 class="wpinv-details-t"><?php echo apply_filters( 'wpinv_email_details_title', __( 'Quote Details', 'invoicing' ) ); ?></h3>
    <table class="table table-bordered table-sm">
        <?php if ( $invoice_number = $invoice->get_number() ) { ?>
            <tr>
                <td><?php _e( 'Quote Number', 'invoicing' ); ?></td>
                <td><a href="<?php echo esc_url( $invoice_url ) ;?>"><?php echo $invoice_number; ?></a></td>
            </tr>
        <?php } ?>
        <tr>
            <td><?php _e( 'Quote Status', 'invoicing' ); ?></td>
            <td><?php echo $invoice->get_status( true ); ?></td>
        </tr>
        <?php if ( $invoice->is_renewal() ) { ?>
        <tr>
            <td><?php _e( 'Parent Quote', 'invoicing' ); ?></td>
        ...
        <?php if ( $invoice_date = $invoice->get_invoice_date( false ) ) { ?>
            <tr>
                <td><?php _e( 'Quote Date', 'invoicing' ); ?></td>
        ...
        <?php if ( $use_taxes && $user_vat_number = $invoice->vat_number ) { ?>
            <tr>
                <td><?php echo wp_sprintf( __( 'Quote %s Number', 'invoicing' ), $vat_name ); ?></td>
